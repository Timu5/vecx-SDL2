...
static void int_update(VIA6522 *VIA)
{
...
uint8_t via_read(VIA6522 *VIA, uint16_t address)
{
	...
	{
	case 0x0:
		data = VIA->read8_port_b(VIA->userdata);
		break;
	case 0x1:
		...
		/* fall through */
	case 0xf:
		data = VIA->read8_port_a(VIA->userdata);
		break;
	case 0x2:
...
void via_write(VIA6522 *VIA, uint16_t address, uint8_t data)
{
	...
	case 0x0:
		VIA->orb = data;
		VIA->write8_port_b(VIA->userdata, data);
	...
	case 0xf:
		VIA->ora = data;
        VIA->write8_port_a(VIA->userdata, data);
		break;
	case 0x2:
		VIA->ddrb = data;
		break;
